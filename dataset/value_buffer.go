...
func NewValueBuffer(values ...float64) *ValueBuffer {
	var tail int
	array := make([]float64, mathutil.MaxInt(len(values), bufferDefaultCapacity))
	if len(values) > 0 {
		copy(array, values)
		tail = len(values)
	}
...
// Instead of allocating a whole new node object for each element, array
// elements are re-used (which saves GC churn).
...
func (b *ValueBuffer) Array() sequence.ArraySequence {
	...
	return sequence.ArraySequence(newArray)
}

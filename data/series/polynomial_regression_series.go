...
var (
	...
	_ data.FirstValuesProvider = (*PolynomialRegressionSeries)(nil)
	_ data.LastValuesProvider  = (*PolynomialRegressionSeries)(nil)
)
...
func (prs PolynomialRegressionSeries) GetStyle() render.Style {
	return prs.Style
}
...
func (prs PolynomialRegressionSeries) Len() int {
	return mathutil.MinInt(prs.GetLimit(), prs.InnerSeries.Len()-prs.GetOffset())
}
...
func (prs *PolynomialRegressionSeries) GetValues(index int) (x, y float64) {
	...
	effectiveIndex := mathutil.MinInt(index+offset, prs.InnerSeries.Len())
...
func (prs *PolynomialRegressionSeries) computeCoefficients() ([]float64, error) {
	...
	return mathutil.PolyRegression(xvalues, yvalues, prs.Degree)
}
...
func (prs *PolynomialRegressionSeries) Render(r render.Renderer, canvasBox render.Box, xrange, yrange data.Range, defaults render.Style) {
	style := prs.Style.InheritFrom(defaults)
	drawLineSeries(r, canvasBox, xrange, yrange, style, prs)
}

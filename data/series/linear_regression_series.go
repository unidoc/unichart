...
var (
	...
	_ data.FirstValuesProvider       = (*LinearRegressionSeries)(nil)
	_ data.LastValuesProvider        = (*LinearRegressionSeries)(nil)
	_ data.LinearCoefficientProvider = (*LinearRegressionSeries)(nil)
)
...
func (lrs LinearRegressionSeries) GetStyle() render.Style {
	return lrs.Style
}
...
func (lrs LinearRegressionSeries) Len() int {
	return mathutil.MinInt(lrs.GetLimit(), lrs.InnerSeries.Len()-lrs.GetOffset())
}
...
func (lrs LinearRegressionSeries) GetEndIndex() int {
	...
	return mathutil.MinInt(windowEnd, innerSeriesLastIndex)
}
...
func (lrs *LinearRegressionSeries) GetValues(index int) (x, y float64) {
	...
	effectiveIndex := mathutil.MinInt(index+offset, lrs.InnerSeries.Len())
...
func (lrs *LinearRegressionSeries) Render(r render.Renderer, canvasBox render.Box, xrange, yrange data.Range, defaults render.Style) {
...
func (lrs *LinearRegressionSeries) computeCoefficients() {
	...
	xvalues := data.NewValueBufferWithCapacity(lrs.Len())
	for index := startIndex; index < endIndex; index++ {
		x, _ := lrs.InnerSeries.GetValues(index)
		xvalues.Enqueue(x)
	}

	lrs.avgx = sequence.NewWrapper(xvalues).Average()
	lrs.stddevx = sequence.NewWrapper(xvalues).StdDev()
